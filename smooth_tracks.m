function [smtracks, smtracklength, avar_k] = smooth_tracks(tracks,kernel,dt)
...
for k = 1:length(kernel)
    ...
    if kernel(k) > 1
        ...
        j = 0;
        smtracks = zeros(0,size(tracks,2));
        smtracklength = [];
        for i = 1:max(tracks(:,5))
            if any(tracks(:,5)==i) && length(tracks(tracks(:,5)==i,1))>=mintracks
                idx_i = find(tracks(:,5)==i);
                xp = tracks(idx_i,1);
                yp = tracks(idx_i,2);
                [x2, range] = gauss_position(xp',kernel(k));
                [y2, ~] = gauss_position(yp',kernel(k));
                [u2, ~] = gauss_velocity(xp',kernel(k),dt);  % combo smoothing + differentiating filter
                ...
                if ~isempty(x2)
                    ...
                    range'+tracks(idx_i(1),7)',Udot_ms2',Vdot_ms2',tracks(idx_i(range),10:end)]; 
        ...
        smtracks = tracks;
        smtracklength = tracklength0;
    end
    avar_k(k) = nanstd(sqrt(smtracks(:,8).^2 + smtracks(:,9).^2))^2;
