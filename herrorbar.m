function hh = herrorbar(x, y, l, r, col, scaleflag)
...
%     if ~isstr(l)
%         r = l;
%         col = [.66 .66 .66]; % gray color
%         scaleflag = 0; % linear scale
%     else
%         col = l;
%         l = y;
%         r = y;
...
if isstr(x) | isstr(y) | isstr(r) | isstr(l)
...
if ~isequal(size(x),size(y)) | ~isequal(size(x),size(l)) | ~isequal(size(x),size(r)),
    if numel(l) == 1 && numel(r) == 1
        ...
        error('The sizes of X, Y, L and R must be the same.');
    end
end
...
if scaleflag == 0
    ...
    tee = interp1(logspace(log10(min(y(:))),log10(max(y(:))),100), ...
        diff(logspace(log10(min(y(:))),log10(max(y(:))),101)), y(:));
end
...
esymbol = '-';
h = plot(xb,yb,'linestyle',esymbol,'color',col,'linewidth',1);
