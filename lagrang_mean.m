function [q_lmean_bin, N_pts, t_lags] = lagrang_mean(smtracks, q1, y_bins, y0m, Tmax, fs, pn)
            ...
            if idx
                ...
                if ~isempty(inbin)
                    for k = 1:length(inbin)
                        ...
                        q_padded = [smtracks(idx(inbin(k):end),q1)', zeros(1, Tmax-length(idx(inbin(k):end)))];
    %                     q_padded2 = q_padded;
                        q_padded(isnan(q_padded)) = 0;
                        q_lmean_bin_p(j,:) = q_lmean_bin_p(j,:) + q_padded;
                        N_padded = [~isnan(q_padded(1:length(idx(inbin(k):end)))), zeros(1, Tmax-length(idx(inbin(k):end)))];
        ...
        for i = 1:max(smtracks(:,5))
            ...
            if idx
                ...
                if ~isempty(inbin)
                    for k = 1:length(inbin)
                        % accumulate track u velocity and number of samples at each time lag (aligned at t0 = 0 lag)
                        q_padded = [zeros(1, Tmax-length(idx(inbin(k):end))), smtracks(idx(inbin(k):end),q1)'];
    %                     q_padded2 = q_padded;
                        q_padded(isnan(q_padded)) = 0;
                        q_lmean_bin_n(j,:) = q_lmean_bin_n(j,:) + q_padded;
                        N_padded = [zeros(1, Tmax-length(idx(inbin(k):end))), ~isnan(q_padded(1:length(idx(inbin(k):end))))];
