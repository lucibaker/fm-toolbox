            ...
            if ~isempty(amb_flag)
                ...
                for j = 1:length(amb_flag)
                    if amb_flag(j) < length(idx_i)
                        ...
                        % choose track with minimum pdd
                        [min_pdd,I] = min([pdd0_mag(amb_flag(j)+1),pdd1_mag(amb_flag(j)+1),pdd2_mag(amb_flag(j)+1),pdd3_mag(amb_flag(j)+1)]);
    %                     if intersect(pz_flag,amb_flag(j))
    %                         [min_pdd,I] = min([pdd0_mag(amb_flag(j)+1),pdd1_mag(amb_flag(j)+1),inf,inf]);  % never change pz sign for rods when pz=0
    % %                         [min_pdd,I] = min([inf,inf,pdd2_mag(amb_flag(j)+1),pdd3_mag(amb_flag(j)+1)]); % force pz sign change for rods when pz=0
    %                     end
